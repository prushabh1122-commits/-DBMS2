...
ACCEPT p_letter CHAR PROMPT 'Enter a starting letter for member name: ';

DECLARE
    CURSOR c_by_letter (p_letter VARCHAR2) IS
        SELECT member_name, course
        FROM   lib_member
        WHERE  UPPER(SUBSTR(member_name,1,1)) = UPPER(p_letter);

    v_letter VARCHAR2(1) := '&p_letter';
BEGIN
    DBMS_OUTPUT.PUT_LINE('Members whose name starts with "' || UPPER(v_letter) || '":');
